...
- [x] **main.ts 上帝模块拆分 + 重渲染保焦保值 + 同步引擎特征测试**（web/refactor）——2026-09-06 完成
  四步走：① 抽 `core/session.ts`（服务器/房间/凭证会话状态与凭证生命周期）与 `core/sync.ts`
  （推送/轮询/断线退避状态机，fetch/定时/DOM 全注入——纯搬移零行为变化）；② 24 例特征测试
  （Node 原生 test runner + 假时钟 + 假网络，零测试框架）锁住版本跳过/dirty 保护/401 与超时分流/
  退避序列等不变式；③ 轮询重建改事件驱动（Session 订阅 → sync.reconfigure，删掉散落各操作尾部的
  手动调用）；④ 重渲染保焦保值（`data-persist-key` + lastRenderedValues 判据），删掉三个弹窗的
  「草稿进 UiState」样板。main.ts 1100 → 832 行（约 450 行为 GmContext 房间操作实现；
  全局可变状态从 ~12 个归零，全部收进 Session / SyncEngine / UiState）。
  测试先后顺序：先纯搬移、立刻补特征测试、之后才做行为微调——③④ 都在测试网织好之后。

- [x] **同步引擎三连 bug：down 态退避卡死 / retryNow 不重置节拍 / 404 房间已删从未退出**
  （web/bug，特征测试与走查抓出）——2026-09-06 完成
  ① 退避翻倍只在 degraded 分支做，进入 down 后无分支处理——pollDelay 卡死 20s，
  且离线状态点一次手动「重试」（pollDelay 复位 5s）后永久 5s 空转，正是 README 明确要避免的
  「服务器已下线时固定 5s 打下去只是空转」；注释「pollDelay 已在 degraded 阶段拉满」从未成立
  （进 down 时只翻到 20000）。修复：down 态继续翻倍至 60s 封顶。
  ② 手动 retryNow 只重置 pollDelay 变量、旧的长间隔定时原封不动，「退避重置回起点」从未落在节拍上；
  重构轮询循环为代次令牌驱动，重试取消旧定时、收尾后按重置值重排。
  ③ README「轮询协议」声称 404 → 房间已删退出（标记 2026-08-30 完成），代码从未实现——
  房间被删后玩家端只会无限退避重试。补上：轮询/启动拉取撞 404 → 停轮询、退出房间并提示
  （与 401 的弹重输分流），端到端验证通过。
