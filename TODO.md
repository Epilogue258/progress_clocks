...
- [ ] **断线三阶段的界面置灰**（`TODO-97427710` 剩余，web/ux）
  推送失败已有退避重试、401 分流与「静默补一次快速重试」；缺的是可见状态：
  1. 首次连线失败补一次快速重试仍失败 → 进「重试中」：置灰全部写操作 + 界面给「重试中」提示
  2. 再连续失败 3 次 → 离线面板，只留「另存为本地」可用，引导落成本地房间继续跑团
  3. 轮询间隔逐步拉长 `5s → 10s → 20s → 40s`，上限 `60s` + 手动「重试」按钮，任一请求成功即解除

  置灰清单与「恢复要有可见反馈」约束见 README「轮询结果的处理」。
  「另存为本地」入口已就位（见 Done 区）。
...
- [x] **「另存为本地」**（`TODO-2d37f13a` 的入口部分）——2026-08-30 完成
  ...
  「离线置灰」部分单独立项，见 Open 区「断线三阶段的界面置灰」。
...
- [x] **轮询协议：结果分类 + 降级 + 恢复**（`TODO-97427710` 的核心）——2026-08-30 完成
  ...
  界面置灰部分单独立项，见 Open 区「断线三阶段的界面置灰」。
