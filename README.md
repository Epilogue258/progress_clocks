...
- **同步**：GM 变更 → 防抖 500ms 全量 JSON 推送（乐观锁 version）；玩家 5s 轮询；启动拉取失败 5s 自动重试（server 重启后前端自动恢复）
- **冲突**：409 → 拉最新替换本地（丢包重做模式，桌游场景足够）；「写锁持有者发疯」只能靠推送前撤销 + 重建数据，工具无法阻止
- **安全边界**：房间名白名单防目录穿越（排除 / \ 与 Windows 保留字符）；1MB 请求体上限；无 SQL 无注入面；version 伪造需写凭证，仅影响游戏数据
